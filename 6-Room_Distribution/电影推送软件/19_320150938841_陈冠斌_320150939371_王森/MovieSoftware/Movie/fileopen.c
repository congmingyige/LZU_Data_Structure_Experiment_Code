...
void fileopen_user_information()
{
    ...
    *(strstr(filename,"MovieSoftware")+14)='\0';
    ...
    if (uMode==1)
    {
        ...
        fscanf(fp_user_information,"%d%d%d",&user[uNumber].Sex,&user[uNumber].Age,&user[uNumber].Preference);
    }
    fclose(fp_user_information);
}

void fileopen_introduce()
{
    long i;
    char filename[100];
    //获得文件路径
    GetModuleFileName(NULL,filename,100);
    *(strstr(filename,"MovieSoftware")+14)='\0';
    strcat(filename,"introduce.txt");
    FILE *fp_introduce=fopen(filename,"r");
    fscanf(fp_introduce,"%ld",&total_introduce);
    fscanf(fp_introduce," ");
    for (i=0;i<total_introduce;i++)
        fgets(introduce[i],100,fp_introduce);
    fclose(fp_introduce);
}

void fileopen_history()
{
    long i;
    char filename[100];
    //获得文件路径
    ...
    *(strstr(filename,"MovieSoftware")+14)='\0';
    ...
    FILE *fp_history=fopen(filename,"r");
    fscanf(fp_history,"%ld",&total_history);
    fscanf(fp_history," ");
    for (i=0;i<total_history;i++)
        fgets(history[i],100,fp_history);
    fclose(fp_history);
}
